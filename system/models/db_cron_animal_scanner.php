...
if (defined('site_path')) {
	require site_path . 'system/models/model_cron.php';
} else {
	require 'model_cron.php';
}

/**
 * Class of database access for Animal Scanner.
...
class Db extends Model_Cron
{
	...
	/**
	 * Returns Subscriber ID and Client ID of an Animal.
	 */
	public function getParentIdsOfAnimal($animalId)
	{
		$sql	= "SELECT subscriberId, client.clientId
			FROM client
			INNER JOIN animal ON animal.clientId = client.clientId
	...
	public function writeAnimalFollowUp($aProc, $cronSession, $animalId, $clientId)
	{
		/**
		...
		$aOwners 			= $this->getOwners(NULL, $clientId);
		...
		foreach ($aOwners as $aOwner) {
			...
			$aAnimals	= $this->getAnimalsOfOwner($animalId, $aOwner['clientId'], $condSpecie, $condGender, $condBooleanJoin, $condEvtBooleanJoin, $condQuantityJoin);
			...
			foreach ($aAnimals as $aAnimal) {
				...
				if ($aProc['recurringPeriod']) {
					 ...
					 */
					$unixSendDate	= $this->getNextSendDate($cronSession, $aProc, $aAnimal['animalId']);
					...
					if ($aProc['procSteps'] == 'group') {
						...
						if ($this->usableAsSendDate($unixSendDate, $aProc['scannerEvalDate'], $aProc['lastUpdateDate'], $animalId, $aProc['procedureId']) == false) {
							/**
						...
						if ($aProc['procSteps'] == 'one') {
							 ...
							 * One-Step Procedure
							 */
							$unixSendDate			= $this->computeDerivedSendDate($aAnimal['animalId'], $aProc['refDateId'], $cronSession, $aProc['offset'], $aProc['anticipation']);
							 ...
							 * Two-Step Procedure
							 */
							$unixSendDateMulti		= $this->computeDerivedSendDate($aAnimal['animalId'], $aProc['refDateId'], $cronSession, $aProc['offset'], $aProc['anticipation']);
							
							if ($this->usableAsSendDate($unixSendDateMulti, $aProc['scannerEvalDate'], $aProc['lastUpdateDate'], $animalId, $aProc['procedureId'])) {
								$unixSendDate		= $unixSendDateMulti;
								$reminderReferenceDate	= $aProc['refDateId'];
								
							} else {
								$unixSendDateSingle		= $this->computeDerivedSendDate($aAnimal['animalId'], $aProc['singleRefDateId'], $cronSession, $aProc['offset'], $aProc['anticipation']);
								
								if ($this->usableAsSendDate($unixSendDateSingle, $aProc['scannerEvalDate'], $aProc['lastUpdateDate'], $animalId, $aProc['procedureId'])) {
				...
				if ($this->evaluateUnless($cronSession, $aAnimal['animalId'], $unixSendDate, $aDtMinuen, $aDtSubtrahen, $aOp, $nDaysUnixValue) == false) {
					// Disregard this Animal because UNLESS condition did not validate.
					++$acc['nX_failedUnlessCond'];
					$acc['nXr_failedUnlessCond'] += $this->deleteFollowUp($aProc['procedureId'], $aAnimal['animalId'], $clientId);
					continue;
				}
		...
		/**
		 * Since this call only processes ONE Animal, 
		 *	we can use Animal saved count to check if any follow-up was written.
		 *	If count = 0, then we can issue a discarding of follow-ups that was
		 *		generated by this Proc to this specific Animal.
		 */
		$nDel		= 0;
		
		if ($acc['nAnimals'] == 0) {
			/**
			 * Proc no longer applies to this animal
			 *	delete existing follow-ups and reminders if any.
			 */
			$nDel	= $this->discardFollowUps($aProc['procedureId'], $animalId);
		}
		
		
		return array($acc, $noChannelOwners, $nDel);
	}
}
