<?php
/**
 * Model file of controller: db_cron_uploadini
 *
...
require site_path . 'system/models/model_common.php';

/**
 * Database access functions of controller: testuploadini
 * @package    db_class
 */
Class Db_Cron_UploadIni Extends Model_Common
{
	/**
		...
		$sql	= "INSERT INTO client
			...
			UPDATE 	clientId 		= LAST_INSERT_ID(clientId),
				...
				country 		= '{$aData['country']}'
					...
					noMessage*/
			";
		$result		= $this->safeExecInsertWithParam($sql, 
			array($aData['clientLastName'], $aData['clientFirstName'], 
				$addr1, $addr2, 
				$aData['clientLastName'], $aData['clientFirstName'], 
				$addr1, $addr2)
		);
	...
	public function saveSubscriberCaption($code, $caption, $languageCode)
	{
		...
		$sql	= "INSERT INTO criteriaLanguage SET
			...
			UPDATE	value		= ?
			";
		return $this->safeExecInsertWithParam($sql, array($code, $caption, $caption));
	}
